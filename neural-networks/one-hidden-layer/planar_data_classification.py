...
def layer_sizes(X, Y, verbose=False):
    '''
    Arguments:
    ...
    n_h -- the size of the hidden layer (set to 4)
    ...
    if(verbose):
        ...
        print('The size of the output layer is: n_y = {}'.format(n_y))

    return (n_x, n_h, n_y)
    
def initialize_parameters(n_x, n_h, n_y, verbose=False):
    '''
    Argument:
...
def main():
    ...
    print('Accuracy of logistic regression: {}% (percentage of correctly labelled datapoints)'.format(float((np.dot(Y,LR_predictions) + np.dot(1-Y,1-LR_predictions))/float(Y.size)*100)))
